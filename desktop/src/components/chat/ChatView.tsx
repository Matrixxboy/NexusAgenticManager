    ...
    if (line.startsWith("```")) {
      ...
      elements.push(
        ...
        >
          ...
          >
            <code style={{ color: "#f2f2f2" }}>{codeLines.join("\n")}</code>
          </pre>
        </div>,
      )
      ...
      elements.push(
        <div key={i} className="nx-divider" style={{ margin: "12px 0" }} />,
      )
    } else if (line.trim()) {
      const formatted = line
        .replace(
          /\*\*(.*?)\*\*/g,
          (_, t) =>
            `<strong style="color:var(--text);font-weight:600">${t}</strong>`,
        )
        .replace(
          /`(.*?)`/g,
          (_, t) =>
            `<code style="font-family:var(--font-mono);background:rgba(0,0,0,0.4);border:1px solid var(--border);padding:2px 6px;border-radius:3px;font-size:11px;color:#f2f2f2">${t}</code>`,
        )
      elements.push(
        <p
          key={i}
...
const QUICK_ACTIONS = [
  ...
  },
  {
    ...
    prompt: "Analyze my skill gap for Senior ML Engineer",
    agent: "COMPASS",
  },
  ...
  {
    label: "Sync GitHub",
    prompt: "sync github and show my open issues",
    agent: "ATLAS",
  },
  {
    label: "Focus Mode",
    prompt: "What single thing should I work on right now?",
    agent: "NEXUS",
  },
]
...
function ChatInput() {
  ...
  const submit = async (overrideMsg?: string) => {
    ...
    try {
      ...
      const { updateLastMessage } = useNexusStore.getState()
      updateLastMessage(
        "⚠️ Backend offline. Run: `uvicorn api.main:app --reload` in /backend",
      )
    } finally {
      setLoading(false)
    }
      ...
      >
        ...
        <button
          ...
          style={{
            ...
            justifyContent: "center",
            background:
              value.trim() && !isLoading ? "var(--accent)" : "var(--surface3)",
            border: "none",
            color: "white",
            fontSize: "16px",
            fontWeight: 700,
        ...
        <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
          {["ATLAS", "ORACLE", "COMPASS", "FORGE"].map((a) => (
            <span
              ...
              className={`chip bg-agent-${a.toLowerCase()} agent-${a.toLowerCase()}`}
            >
              {a}
            </span>
          ))}
